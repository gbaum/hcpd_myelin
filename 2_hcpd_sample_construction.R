...
# HCD_myelin_maps_cifti <- read_cifti("/ncf/hcp/data/analyses/myelin/data/myelinMaps_March2021/HCDMyelin/Partial.All.MyelinMap_IndPseudoCorr_Reg_MSMAll.32k_fs_LR.dscalar.nii")
...
#########################################################
## REMOVE SUBJECTS YOUNGER THAN 8 DUE TO POOR SAMPLING ##
#########################################################
brain_data <- subset(hcpd_data, hcpd_data$Age > 8)
dim(brain_data)

##############################################
## REMOVE SUBJECTS FROM MATT GLASSER'S LIST ##
##############################################
rm_idx <- which(brain_data$subject_id=="HCD1784065" | brain_data$subject_id=="HCD2643456")

brain_data <- brain_data[-rm_idx,]
dim(brain_data)
...
tmp_myelin <-as.data.frame(brain_data[myelin_col_index])
mean_wholebrain_T1wT2w <- rowMeans(tmp_myelin)
brain_data$mean_wholebrain_T1wT2w <- mean_wholebrain_T1wT2w
hist(brain_data$mean_wholebrain_T1wT2w, col="slategray3")

## Create Threshold Based on Wholebrain T1wT2w ##
high_wholebrain_thresh <- mean(brain_data$mean_wholebrain_T1wT2w) + (4*sd(brain_data$mean_wholebrain_T1wT2w))
low_wholebrain_thresh <- mean(brain_data$mean_wholebrain_T1wT2w) - (4*sd(brain_data$mean_wholebrain_T1wT2w))

myelin_thresh_idx <- which(brain_data$mean_wholebrain_T1wT2w > high_wholebrain_thresh | brain_data$mean_wholebrain_T1wT2w < low_wholebrain_thresh)
length(myelin_thresh_idx)
brain_data$subject_id[myelin_thresh_idx]

#######################################################
## REMOVE SUBEJECTS WITH WHOLEBRAIN T1w/T2w OUTLIERS ##
#######################################################
brain_data <- brain_data[-myelin_thresh_idx,]
dim(brain_data)

hist(brain_data$mean_wholebrain_T1wT2w, col="firebrick4")


####################################
## Read in cortical thickness maps ##
#####################################
nsub <- dim(brain_data)[1]
nreg <- 360
glasser_thickness_maps <- as.data.frame(array(rep(NA, nsub*nreg), dim=c(nsub, nreg)))

for(i in 1:nsub) {
  sub_id <- brain_data$subject_id[i]
...
## Merge with brain_data
brain_data <- cbind(brain_data, glasser_thickness_maps)

# Visualize Sample Characteristics ----

## Age by Sex: Proportion
age_sex_plot <- ggplot(brain_data, aes(x=age_floor, fill=Sex, col=Sex)) + geom_bar(position = "fill", alpha=0.7) + scale_y_continuous(labels = scales::percent)
age_sex_plot + gbtheme + scale_fill_manual(values = c("#424C6D",  "#CE7B5B")) + scale_colour_manual(values = c("#424C6D",  "#CE7B5B"))  

## Age by Sex: Count
age_sex_plot <- ggplot(brain_data, aes(x=age_floor, fill=Sex, col=Sex)) + geom_bar(alpha=0.7)
age_sex_plot + gbtheme + scale_fill_manual(values = c("#424C6D",  "#CE7B5B")) + scale_colour_manual(values = c("#424C6D",  "#CE7B5B"))  

## Age by SES: Proportion
## Age by SES
rm_idx <- which(is.na(brain_data$SES_RLVL)==TRUE)
ses_data <- brain_data[-rm_idx,]
...
## Export data as csv
# write.csv(brain_data, "/ncf/hcp/data/analyses/myelin/data/subject_demographics/n630_hcpd_Regressed_myelin_GlasserMMP_Yeo7_ColeAnt12.csv", row.names=FALSE)
write.csv(brain_data, "/ncf/hcp/data/analyses/myelin/data/subject_demographics/n628_hcpd_newCorr_myelin_Aug2021.csv", row.names=FALSE)
write.table(brain_data$subject_id, "/ncf/hcp/data/analyses/myelin/subject_list/n628_hcpd_newCorr_subject_list.txt", row.names=FALSE, col.names=FALSE, quote = FALSE)
# write.table(hcpd_data$subject_id, "/ncf/hcp/data/analyses/myelin/subject_list/n655_hcpd_subject_list.txt", row.names=FALSE, col.names=FALSE, quote = FALSE)
...
#########################################
## Organize NIH Toolbox Cognitive Data ##
##########################################
rm(list = ls())

###########################
## New demographics file ##
###########################
hcpd_demogs <- read.csv("/ncf/hcp/data/analyses/myelin/data/NIH_cognition_toolbox_data/GB_HCPD_COMBINED20200608.csv")

## Load brain data for merging (if exists, can ignore previous lines) 
#  load("/Users/localadmin/Desktop/hcpd_n632_TFcorr_myelin_GlasserMMP_Yeo7_data.Rdata")

################################
## NIH Toolbox Cognition Data ##
################################
cog_long <- read.csv("/ncf/hcp/data/analyses/myelin/data/NIH_cognition_toolbox_data/GB_RePull_SCORES_Data_With_Duplicates_Removed.csv")

## Set scores as numeric
cog_long$RawScore <- as.numeric(cog_long$RawScore)
cog_long$Uncorrected.Standard.Score <- as.numeric(cog_long$Uncorrected.Standard.Score)
cog_long$Age.Corrected.Standard.Score <- as.numeric(cog_long$Age.Corrected.Standard.Score)
cog_long$National.Percentile..age.adjusted. <- as.numeric(cog_long$National.Percentile..age.adjusted.)

## Transform data from long to wide format
RawScore_data <- dcast(cog_long, subject_id ~ Inst, fun=mean, value.var="RawScore")
UncStandardScore_data <- dcast(cog_long, subject_id ~ Inst, fun=mean, value.var="Uncorrected.Standard.Score")
AgeCorrStandardScore_data <- dcast(cog_long, subject_id ~ Inst, fun=mean, value.var="Age.Corrected.Standard.Score")
Percentile_data <- dcast(cog_long, subject_id ~ Inst, fun=mean, value.var="National.Percentile..age.adjusted.")

## Merge with demographics
cog_data <- merge(hcpd_demogs, UncStandardScore_data, by="subject_id")

cog_data <- merge(hcpd_demogs, AgeCorrStandardScore_data, by="subject_id")

## Create syntactical names for GAMs ##
names(cog_data) <- make.names(names(cog_data), unique = TRUE) 


## Remove subjects with multiple Toolbox scores for same task
library(dplyr)
omit <- c("HCD0101006", "HCD0193845", "HCD0330122", "HCD0541034", "HCD0914956", "HCD1322835", "HCD2035837", "HCD2095148", "HCD2109234", "HCD2152942", "HCD2333340", "HCD2473356", "HCD2740454", "HCD2741961", "HCD2970269")
cog_data <- filter(cog_data, !(subject_id %in% omit))

dim(cog_data)

######################################################################
## Create single score for Toolbox tasks with multiple Age versions ##
######################################################################
## Create single score for Toolbox tasks with multiple Age versions 
cog_data$Flanker <- 0
cog_data$Dimensional_Card_Sort <- 0
cog_data$Working_Memory <- 0
cog_data$Processing_Speed <- 0

nsub <- dim(cog_data)[1]


for(i in 1:nsub) {
  tmp_age <- cog_data$age[i]
  
  if (tmp_age < 12 & tmp_age > 8) {
    cog_data$Flanker[i] <- cog_data$NIH.Toolbox.Flanker.Inhibitory.Control.and.Attention.Test.Ages.8.11.v2.1[i]
    cog_data$Dimensional_Card_Sort[i] <- cog_data$NIH.Toolbox.Dimensional.Change.Card.Sort.Test.Ages.8.11.v2.1[i]
    
  } else {
    cog_data$Flanker[i] <- cog_data$NIH.Toolbox.Flanker.Inhibitory.Control.and.Attention.Test.Age.12..v2.1[i]
    cog_data$Dimensional_Card_Sort[i] <- cog_data$NIH.Toolbox.Flanker.Inhibitory.Control.and.Attention.Test.Age.12..v2.1[i]
  }
}
...
for(i in 1:nsub) {
  ...
  if (tmp_age < 7) {
    cog_data$Working_Memory[i] <- cog_data$NIH.Toolbox.List.Sorting.Working.Memory.Test.Ages.3.6.v2.1[i]
    cog_data$Processing_Speed[i] <- cog_data$NIH.Toolbox.Pattern.Comparison.Processing.Speed.Test.Ages.3...6.v2.1[i]
    
  } else {
    cog_data$Working_Memory[i] <- cog_data$NIH.Toolbox.List.Sorting.Working.Memory.Test.Age.7..v2.1[i]
    cog_data$Processing_Speed[i] <- cog_data$NIH.Toolbox.Pattern.Comparison.Processing.Speed.Test.Age.7..v2.1[i]
  }
}
...
for(i in 1:nsub) {
  ...
  if (tmp_age < 8) {
    cog_data$Dimensional_Card_Sort[i] <- cog_data$NIH.Toolbox.Dimensional.Change.Card.Sort.Test.Ages.3.7.v2.1[i]
    cog_data$Flanker[i] <- cog_data$NIH.Toolbox.Flanker.Inhibitory.Control.and.Attention.Test.Ages.3.7.v2.1[i]
    
  } else {
    cog_data$Dimensional_Card_Sort[i] <- cog_data$Dimensional_Card_Sort[i]
    cog_data$Flanker[i] <- cog_data$Flanker[i]
  }
}
...
#########################################################
dim(cog_data)
cog_data <- subset(cog_data, cog_data$age > 8)
dim(cog_data)
...
## Export data as csv ##
########################
write.csv(cog_data, "/ncf/hcp/data/analyses/myelin/data/NIH_cognition_toolbox_data/n1023_hcpd_UncorrectedStandardScores.csv", row.names=FALSE)



######################################################
## Remove subjects who are missing Cognitive Scores ##
######################################################
final_df <- cog_data[!is.na(cog_data$Working_Memory) | !is.na(cog_data$Flanker) | !is.na(cog_data$Dimensional_Card_Sort) | !is.na(cog_data$Processing_Speed), ]
dim(final_df)

final_df <- final_df[!is.na(final_df$Flanker), ]
dim(final_df)

########################
## Export data as csv ##
########################
write.csv(final_df, "~/Desktop/n599_hcpd_TFcorr_myelin_UncorrectedStandardScores.csv", row.names=FALSE)
...
## Age by Sex
age_sex_plot <- ggplot(cog_data, aes(x=Age, col=Sex)) + geom_histogram(aes(fill=Sex, alpha=0.4), bins=20)
age_sex_plot + gbtheme + scale_fill_manual(values = c("#424C6D",  "#CE7B5B")) + scale_colour_manual(values = c("#424C6D",  "#CE7B5B"))  


ggplot(cog_data,aes(x=Age, col=Sex)) +
  geom_histogram(aes(fill=Sex, position="dodge",alpha=0.5,binwidth=0.25)) + gbtheme + scale_fill_manual(values = c("#424C6D",  "#CE7B5B")) + scale_colour_manual(values = c("#424C6D",  "#CE7B5B"))  

## Age by SES
rm_idx <- which(is.na(cog_data$SES_RLVL)==TRUE)
ses_data <- cog_data[-rm_idx,]
age_ses_plot <- ggplot(ses_data, aes(x=Age, col=SES_RLVL, na.rm=TRUE)) + geom_histogram(aes(fill=SES_RLVL, alpha=0.4), bins=20, na.rm=TRUE) 
...
## Export data as csv ##
########################
write.csv(cog_data, "~/Desktop/n759_hcpd_Cognition_UncorrectedStandardScores.csv", row.names=FALSE)

hist(cog_data$Flanker, col="coral2")

hist(cog_data$Working_Memory, col="coral2")

hist(cog_data$Dimensional_Card_Sort, col="coral2")

hist(cog_data$Processing_Speed, col="coral2")
